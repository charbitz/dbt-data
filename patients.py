...
df = pd.read_csv("/mnt/seagate/DBT/duke-dbt-data_github/data/manifest-1617905855234/BCS-DBT labels-new-v0.csv")
...
for iter in range(df.shape[0]):
  elem = str(df.loc[iter]["PatientID"]) + str(df.loc[iter]["StudyUID"]) + str(df.loc[iter]["View"])
  if iter >= 200:
    # print("elem:", elem)
